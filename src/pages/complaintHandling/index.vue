<template>
  <div class="travel-index">
    <div class="content-title">投诉处理</div>
    ...
    <!-- 遮罩层 -->
    <div class="model-box" v-if="dialogVisible">
      <div class="left-box" :class="dialogIndex * 1 > 5 ? 'active' : ''"></div>
      <div class="center-box" :class="dialogIndex * 1 <= 5 || dialogIndex * 1 >= 12 ? 'active' : ''"></div>
      <div class="right-box" :class="dialogIndex * 1 < 12 ? 'active' : ''"></div>
    </div>
  </div>
</template>
...
const { dialogIndex,dialogVisible } = toRefs(dialogStore)


const png1 = ref("")
png1.value = new URL('../../assets/image/TransportationAndTravel/气象灾害预警信号图标_slices/Property 1=干旱, Property 2=1级红色.png', import.meta.url).href;
...
.travel-index {
  ...
  background-image: url("assets/image/complaintHandling/组 43229.jpg");
  ...
  &::after {
    background-image: url("assets/image/complaintHandling/外框.png");
    background-size: 100% 100%;
    background-position: center;
    background-repeat: no-repeat;
    position: absolute;
  ...
  .content-title {
    ...
    letter-spacing: 12px;
    // font-style: italic;
    // text-shadow: 4px 8px 12px rgba(8, 30, 57, 0.5);
  ...
  .model-box {
    ...
    &>.active {
      pointer-events: none;
      background: rgba($color: #09132d, $alpha: .8);
    }
